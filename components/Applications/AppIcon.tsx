...
const AppIcon = (
  ...
  const baseClasses =
    "flex items-center rounded-md cursor-pointer transition-all duration-150 ease-in-out focus:outline-none focus:ring-1";
  ...
  const iconIsUrl =
    ...
    (app.icon.startsWith("http") || app.icon.startsWith("data:image"));
  ...
  if (showTitle) {
    childrenElements.push(
      React.createElement("span", { className: "text-sm truncate" }, app.name),
    );
  }
  ...
  if (!showTitle && isActive) {
    childrenElements.push(
      React.createElement("div", {
        className:
          "absolute bottom-0 left-1/2 -translate-x-1/2 w-4/5 h-[3px] rounded-t-sm",
  ...
  return React.createElement(
    "button",
    {
      ...
      style: dynamicStyle,
      onMouseEnter: (e) => {
        if (showTitle || (!isActive && !isMinimized))
          e.currentTarget.style.backgroundColor = "var(--secondary-bg)";
      }, // Hover for StartMenu & inactive Taskbar
      onMouseLeave: (e) => {
        if (showTitle || (!isActive && !isMinimized))
          e.currentTarget.style.backgroundColor = "transparent";
      }, // Reset for StartMenu & inactive Taskbar
    },
    ...childrenElements,
  );
};
